...
class GetEventListTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.auth_user = ('testercc', 'testercc123')

    def test_get_event_list_auth_null(self):
        ''' auth为空 '''
        r = requests.get(self.base_url, params={'eid': ''})   # 不传入auth的值
        result = r.json()
    ...
    def test_get_event_list_eid_success(self):
        ...
        self.assertEqual(result['data']['name'], u'接口测试初级指导1')
        self.assertEqual(result['data']['address'], u'会议室1')
    ...
    def test_get_event_list_name_find(self):
        ''' 关键字‘发布会’模糊查询 '''
        r = requests.get(self.base_url, auth=self.auth_user, params={'name': '指导1'})
        result = r.json()
        self.assertEqual(result['status'], 200)
        self.assertEqual(result['message'], 'success')
        self.assertEqual(result['data'][0]['name'],u'接口测试初级指导1')
        self.assertEqual(result['data'][0]['address'], u'会议室1')
